        ...
        <div class="col-md-10">
            <div class="card shadow">
                <div class="card-header bg-white py-3">
                    <h4 class="card-title mb-0">Edit Purchase Order: {{ $order->order_number }}</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('procurement.orders.update', $order) }}" method="POST" id="orderForm">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control @error('shipping_address') is-invalid @enderror" 
                                              id="shipping_address" name="shipping_address" rows="2" required>{{ old('shipping_address', $order->shipping_address) }}</textarea>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('procurement.orders.show', $order) }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const oldItems = @json(old('items', $order->items->map(function($i) { return ['material_id' => $i->material_id, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price]; })->toArray()));
